...
$user_email = $_SESSION['user_email'];
      ...
      // echo $key;
      $sqlGetQuotationToEdit = "SELECT quotationdetails.invoiceTitle, quotationdetails.fromNameInvoice, quotationdetails.fromEmailInvoice, quotationdetails.address1FromInvoice,   quotationdetails.phoneFromInvoice, quotationdetails.refNumberFromInvoice, quotationdetails.dateFromInvoice, quotationdetails.clientNameFromInvoice, quotationdetails.clientEmailFromInvoice, quotationdetails.clientAddressFromInvoice, quotationdetails.user_identification, quotationdetails.paymentInstructionsAddNotes, quotationdetails.typeTaxInvoice, quotationdetails.myPercent, quotationdetails.amountDiscount, quotationdetails.myPercentDiscount, quotationdetails.currencyContInvoice, quotationdetails.subTotal, quotationdetails.taxValue, quotationdetails.discount, quotationdetails.discountInvoice, quotationdetails.pickedColor, quotationdetails.balance FROM quotationdetails WHERE quotationdetails.invoiceIdentifier='$key' AND quotationdetails.user_identification=$user_identification";
      
      $resultGetQuotationToEdit = mysqli_query($conn, $sqlGetQuotationToEdit);
      $GetQuotationToEdit = mysqli_fetch_all($resultGetQuotationToEdit, MYSQLI_ASSOC);
      // print_r($GetQuotationToEdit);
      $QuotationValues = $GetQuotationToEdit;
      if(count($QuotationValues) === 0){
      ...
      if (mysqli_query($conn, $sqlGetQuotationToEdit)) {
        // echo "New record created successfully";
       } else {
          echo "Error: " . $sqlGetQuotationToEdit . "<br>" . mysqli_error($conn);
          // exit;
        }
      // print_r($GetQuotationToEdit);
      $QuotationValues = $GetQuotationToEdit;
      // print_r($QuotationValues);
     
      // exit;
      $sqlGetQuotationToEdit = "SELECT quotationproducteach.description, quotationproducteach.priceInvoice, quotationproducteach.qtyInvoice, quotationproducteach.amountPerItem, quotationproducteach.descriptionRowInvoiceId  FROM quotationdetails  INNER JOIN quotationproducteach ON quotationdetails.invoiceIdentifier = quotationproducteach.invoiceIdentifier WHERE quotationdetails.invoiceIdentifier='$key' AND quotationproducteach.user_identification=$user_identification";
      ...
      if (mysqli_query($conn, $sqlGetQuotationToEdit)) {
        ...
        // exit;
        $ProductValues = $GetQuotationToEdit;
    ...
    // if (mysqli_query($conn, $sqlLogoUrl)) {
    //   // echo "New record created successfully";
    //  } else {
  ...
  $sqlGetQuotationCount = "SELECT  quotationdetails.refNumberFromInvoice FROM quotationdetails WHERE quotationdetails.user_identification=$user_identification";
      
  $resultGetQuotationCount = mysqli_query($conn, $sqlGetQuotationCount);
  $GetQuotationCount = mysqli_fetch_all($resultGetQuotationCount, MYSQLI_ASSOC);
  $count =  count($GetQuotationCount);
...
<div style="width: 100%; display:none; height: 100%; position: fixed; background: rgba(255,255,255,0.6); top: 0;" class="loadCont" >
  <img style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); opacity: 0.6" src="player.gif" alt="">
    ...
    <!-- <input id="hidden" type="hidden" value="<?php echo $user_identification ?>"> -->
              <!-- SAVE TO DATABASE -->
              <div class="heading" style="text-align: center; font-size: 40px; color: #477fae; font-family: 'Lobster' "><h1>Design a Quotation</h1></div>
    ...
    <?php if ($QuotationValues[0]['clientEmailFromInvoice'] == "" && $QuotationValues[0]['clientNameFromInvoice'] == "" && $QuotationValues[0]['clientAddressFromInvoice'] == "") {?>
      ...
      <button  data-logo="<?php echo $logoUrl[0]['logo_url'] ?>" id="download" onclick="downloadPDFHandler(this.className, 'quotation')">Download as PDF</button>

    <?php } else{?>
      <button  class="prev" href="#" onclick="previewQuotation(this.id)">Preview</button>
      <button  id="email" data-client="<?php echo $QuotationValues[0]['clientEmailFromInvoice'] ?>" onclick="emailHandler(this.className, 'quotation')">Email Client</button>
      <button  data-logo="<?php echo $logoUrl[0]['logo_url'] ?>" id="download" onclick="downloadPDFHandler(this.className, 'quotation')">Download as PDF</button>

    <?php }?>
<?php } else {?>
  <button class="prev" href="#" onclick="previewQuotation(this.id)">Preview</button>
      <button data-client="" id="email"  onclick="emailHandler(this.className, 'quotation')">Email Client</button>
      <button data-logo="<?php echo $logoUrl[0]['logo_url'] ?>" id="download" onclick="downloadPDFHandler(this.className, 'quotation')">Download as PDF</button>
 <?php }?>
    </div>
       <div class="invoiceGeneratorCont">
         ...
         <?php if($determinant){?>
          ...
          <div id="topDesignInvoice" style="background: <?php echo $QuotationValues[0]['pickedColor']?>"></div>
                <?php } else {?>
            ...
            <div class="headingLogoInvoice">
              ...
              <?php if($determinant){?>
                      ...
                      <input id="invoicetitle" type="text" value="<?php echo $QuotationValues[0]['invoiceTitle'] ?>" name="invoiceTitle">
              <?php } else {?>
                <input id="invoicetitle" type="text" value="QUOTATION" name="invoiceTitle">
                <?php }?>
              ...
              <div class="invoiceLogo">
                ...
                <?php if((count($logoUrl)) ==0){?>
                    <p style="padding: 10%; border: none; min-width: 150px; position: abosolute; top: 50%; left: 50%;  border: 1px solid #999; cursor: pointer;  outline: none; ">Upload Your Logo</p>
            ...
            <div class="fromToInvoiceCont">
              ...
              <div class="personalDets">
                      <?php if($determinant){?>
                        ...
                        <input type="text" id="fromNameInvoice" onchange="onChangeHandler()" placeholder="Enter Your Name" value="<?php echo $QuotationValues[0]['fromNameInvoice'] ?>" name="name">
                      <?php } else {?>
                        <label for="fromNameInvoice">
...
<i class="fas fa-pen"></i>
                ...
                <input type="email" placeholder="Enter Email Address" onchange="onChangeHandler()" id="fromEmailInvoice" value="<?php echo $QuotationValues[0]['fromEmailInvoice'] ?>" name="email">
                <?php } else {?>
...
<i class="fas fa-pen"></i>
                ...
                <input type="text" value="<?php echo $QuotationValues[0]['address1FromInvoice'] ?>" id="address1FromInvoice" onchange="onChangeHandler()" placeholder="Enter  residential address" name="address1">
                <?php } else {?>
...
<i class="fas fa-pen"></i>
                ...
                <input type="text" value="<?php echo $QuotationValues[0]['phoneFromInvoice'] ?>" onchange="onChangeHandler()" id="phoneFromInvoice" placeholder="Phone" name="phone">
                <?php } else {?>
                  <label for="phoneFromInvoice">
...
<i class="fas fa-pen"></i>
          ...
          <i class="fas fa-pen"></i>
                  ...
                  <input onchange="onChangeHandler()" type="text" id="clientNameFromInvoice" placeholder="Client Name" value="<?php echo $QuotationValues[0]['clientNameFromInvoice'] ?>" name="clientname">
                <?php } else {?>
                  <label for="clientNameFromInvoice">
...
<i class="fas fa-pen"></i>
                  ...
                  <input onchange="onChangeHandler()" type="email" value="<?php echo $QuotationValues[0]['clientEmailFromInvoice'] ?>" id="clientEmailFromInvoice" placeholder="Client Email" name="clientemail">
                <?php } else {?>
...
<i class="fas fa-pen"></i>
                  ...
                  <input onchange="onChangeHandler()" type="text" value="<?php echo $QuotationValues[0]['clientAddressFromInvoice']?>" id="clientAddressFromInvoice" placeholder="Client Address" name="clientaddress">
                <?php } else {?>
                  <label for="clientAddressFromInvoice">
...
<i class="fas fa-pen"></i>
                  ...
                  <?php }?>
                      ...
                      <div id="invoiceDueDateTableCont"> 
                          ...
                          <table id="invoiceDueDateTable" cellpadding="10px">
                              <tr>
                              <?php if($determinant){?>
                                <th data-src="okay" id="bamount"  colspan="2"><?php echo $QuotationValues[0]['currencyContInvoice'] .  " " . number_format($QuotationValues[0]['balance'], 2, '.', '')?></th>
                                      <?php } else {?>
                              ...
                              <tr>
                                <td><div class="">
                                <?php if($determinant){?>
                              <input type="text" id="refNumberFromInvoice" onchange="onChangeHandler()" value="<?php echo $QuotationValues[0]['refNumberFromInvoice']?>" placeholder="Reference Number" name="refnumber">
                              <?php } else {?>
                                <input type="text" id="refNumberFromInvoice" onchange="onChangeHandler()" value="#QU-<?php echo $count + 1?>" placeholder="Reference Number" name="refnumber">
                                <?php }?>
                            ...
                            <?php if($determinant){?>
                              <input id="dateFromInvoice" onchange="onChangeHandler()" type="date" value="<?php echo $QuotationValues[0]['dateFromInvoice']?>" placeholder="Date" name="date">
            ...
            <!-- End of From and To div -->
             ...
             <div class="descriptionHeaderForMobile">
               <h2 style="color: <?php echo $QuotationValues[0]['pickedColor']?> ">Enter Your Product Details</h2>
             </div>
             <div class="descriptionContainerInvoice">
             <?php if($determinant){?>
              <div id="descriptionHeaderInvoice" style="background: <?php echo $QuotationValues[0]['pickedColor']?>">
                <?php } else {?>
                      ...
                      <?php foreach($ProductValues as $p) {?>
                        ...
                        <div class="descriptionRowInvoice" data-src="" id="<?php echo $p['descriptionRowInvoiceId'] ?>">
                                  ...
                                  <div id="amountInvoiceInput" class="amountInvoiceInput">
                                    <h6><?php echo $QuotationValues[0]['currencyContInvoice'] . " " . number_format((float)$p['amountPerItem'], 2, '.', '');  ?></h6>
                                  </div>
                                  <div class="cancelItemInvoice">
                                  <?php if($determinant){?>
                                    <i class="fas fa-trash closeProductDescInvoice" style="color:  <?php echo $QuotationValues[0]['pickedColor']?>"></i> <span class="deleteTrashInvoice" style="padding: 2px 8px; font-size: 17px; color: <?php echo $QuotationValues[0]['pickedColor'] ?>;">Delete Item</span> 
                                      <?php } else {?>
            ...
            <?php if($determinant){?>
              <i class="fas fa-plus-circle" style="color: <?php echo $QuotationValues[0]['pickedColor']?>" onclick="addNewProductHandler()" id="addDescInvoice" title="Add new product To Invoice"></i>
            <?php } else {?>
            ...
            <div id="productSummaryInvoice">
              ...
              <?php if($determinant){?>
                <li id="subList" data-src=""><h5>Subtotal</h5> <h5 id="subtotalValueInvoice"><?php echo $QuotationValues[0]['currencyContInvoice'] . " " . number_format((float)$QuotationValues[0]['subTotal'], 2, '.', '') ?></h5></li>
                  
                  <?php if($QuotationValues[0]['discountInvoice'] == "Percent"){?>
                    <li><h5>Discount(<span id="discountTag"><?php echo $QuotationValues[0]['myPercentDiscount']?>%</span>)</h5> <h5 id="discountValueInvoice"><?php echo $QuotationValues[0]['currencyContInvoice'] . " " . number_format((float)$QuotationValues[0]['discount'], 2, '.', '') ?></h5></li>
                  <?php }  else if($QuotationValues[0]['discountInvoice'] == "Flat Amount") {?>
                    <li><h5>Discount(<span id="discountTag"><?php echo $QuotationValues[0]['currencyContInvoice'] . $QuotationValues[0]['amountDiscount']?></span>)</h5> <h5 id="discountValueInvoice"><?php echo $QuotationValues[0]['currencyContInvoice'] . " " . number_format((float)$QuotationValues[0]['discount'], 2, '.', '') ?></h5></li>
                    <?php }  else if($QuotationValues[0]['discountInvoice'] == "None") {?>
                    <li><h5>Discount(<span id="discountTag"><?php echo '0%'?></span>)</h5> <h5 id="discountValueInvoice"><?php echo $QuotationValues[0]['currencyContInvoice'] . " " . number_format((float)$QuotationValues[0]['discount'], 2, '.', '') ?></h5></li>
                  <?php } ?>

                <li><h5>Tax(<span id="taxPercentTag"><?php echo $QuotationValues[0]['myPercent']?>%</span>)</h5> <h5 id="taxValueInvoice"><?php echo $QuotationValues[0]['currencyContInvoice'] . " " . number_format((float)$QuotationValues[0]['taxValue'], 2, '.', '') ?></h5></li>

                <!-- <li> <h5>Total</h5> <h5>$ 0.00</h5></li> -->
                <li><h5>Balance Due</h5> <h5 id="balanceDueValueInvoice"><?php echo $QuotationValues[0]['currencyContInvoice'] . " " . number_format((float)$QuotationValues[0]['balance'], 2, '.', '') ?></h5></li>
                <?php } else {?>
                  ...
                  <div class="paymentInstructionsAddNotes">
                    ...
                    <?php if($determinant){?>
                      <textarea onchange="onChangeHandler()" name="" id="" cols="30" rows="10" placeholder="NOTES - Add any additional notes pertaining to the transaction">
                        <?php echo $QuotationValues[0]['paymentInstructionsAddNotes'] ?>
                    </textarea>
                <?php } else {?>
         ...
         <div class="invoiceFormController">
                ...
                <div class="colorControllerInvoice">
                  ...
                  <input onchange="onChangeHandler()"  type='color' id="colorPickerInvoice" name='color2' value='<?php echo $QuotationValues[0]['pickedColor']?>' />
                <?php } else {?>
                ...
                <div class="taxControllerInvoice">
                 ...
                 <div id="taxControlContainer">
                    <div id="customSelectTypeTax">
                      ...
                      <?php if($determinant){?>
                        <?php if ($QuotationValues[0]['typeTaxInvoice'] == "On Total")  {?>
                          ...
                          <select value="OnTotal" onchange="onChangeHandler()" name="type" id="typeTaxInvoice">
                            ...
                            <div style="width: 100%; position: relative"><label for="myPercent" style="margin-top:20px;">Rate</label> 
                              <input onkeyup='onChangeHandler()' type="number" min="0" max="100" value="<?php echo number_format((float)$QuotationValues[0]['myPercent'], 2, '.', '' )?>" step="0.01" id="myPercent"/>
                              ...
                              <?php } else if($QuotationValues[0]['typeTaxInvoice'] == "Deducted") {?>
          ...
          <div class="discountControllerInvoice">
                 ...
                 <div id="discountControlContainer">
                    ...
                    <?php if($determinant){?>
                      ...
                      <input type="number" onkeyup="onChangeHandler()" min="0" max="100" value="<?php echo $QuotationValues[0]['myPercentDiscount']?>" step="0.01" id="myPercentDiscount"/>
                      <span style="position: absolute;
                      bottom: 7px;
                      right: 30px;
                      font-size: 18px;">%</span>
                      ...
                      <?php } else if($QuotationValues[0]['discountInvoice'] == 'Flat Amount'){?>
                        ...
                        <input type="number" name="amount" onkeyup="onChangeHandler()" id="amountDiscount" placeholder="Amount" step="1.00" value="<?php echo number_format((float)$QuotationValues[0]['amountDiscount'], 2, '.', '')?>"/>
                      </div> 
                ...
                <!-- BEGINNING OF CURRENCY CONTROLLER -->
                      ...
                      <div class="currencyController" style="margin-top: 50px; margin-bottom: 90px; ">
                        ...
                        <?php if($determinant){?>
                          <select data-src="<?php echo $QuotationValues[0]['currencyContInvoice']?>" onchange="onCurrencyChangeHandler(this.id)" name="" id="currencyContInvoice" style="width: 100%; padding: 10px;">
